import { Processor, State, ProcessorState as PS, Processor as P, PipelineStep } from "./types.js";
...
export namespace ProcessorState {
  export const newState = <T>(
    ...
    // let vals = [0, 0, 440, 466, 494, 523, 554, 587, 622, 659, 698, 740, 784, 831, 880]
    // let vals = [0,0,0,0];

    processor.registerNames.forEach((name, index) => {
      if (processor.defaultRegisters != undefined) {
        registers[name] = processor.defaultRegisters[index]
  ...
  export const getRegisters = <T>(ps: PS<T>): Record<string, number> => {
    return ps.state.registers;
  };
  ...
  export const setMemory = <T>(ps: PS<T>, values: Array<number>) => {
    ...
    console.log(ps.state.memory);
  };

  export const setState = <T>(ps: PS<T>, psT: PS<T>) => {
    ps.state = psT.state;
